...
async function initializeAgent() {
  try {
    ...
    logger.info('Agent initialized for dashboard');
  } catch (error) {
    logger.error('Failed to initialize agent for dashboard:', error);
  }
}
  ...
  try {
    if (!agent) {
      return res.json({ 
        ...
        apiHealth: false, 
        strategies: [], 
        apiKey: 'Not initialized',
        maxPositionSize: 0
      });
    }
    
    const status = agent.getStatus();
    const apiHealth = await agent.client.healthCheck();
...
app.post('/api/stop', async (req, res) => {
  try {
    if (agent) {
      ...
      res.json({ success: true, message: 'Agent was not running' });
    }
  } catch (error) {
...
app.get('/api/trades', async (req, res) => {
  try {
    ...
    const trades = await agent.getTradeHistory(20);
    res.json(trades);
  } catch (error) {
